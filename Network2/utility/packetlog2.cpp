...
bool cPacketLog2::Read(const StrPath &fileName)
{
	Clear();
...
bool cPacketLog2::ReadPacket(const uint64 beginDateTime, const uint64 endDateTime
...
bool cPacketLog2::SetTimeLine(const uint64 dateTime)
{
...
cDateTimeRange cPacketLog2::GetTimeRange(const StrPath &fileName)
{
	using namespace std;
...
// is log header?
bool cPacketLog2::IsLogHeader(const BYTE *buffer)
{
...
bool cPacketLog2::IsValidDateTime(const uint64 dateTime)
{
	return (dateTime > 20210101000000000) && (dateTime < 21210101000000000);
}


void cPacketLog2::Clear()
{
	m_readPos = 0;
	m_cursor = BUFFER_SIZE;
	m_isEof = false;
...
bool cPacketLog2::Write(const char *fileName, const sPacketLogData &logData)
{
