...
m_surrogate       (0),
m_mouseInside     (false),
m_fullscreen      ( ((style & Style::Fullscreen) > 0)? true : false),
...
void makeOpaque(HWND hwnd)
{
	SetWindowLongPtr(hwnd, GWL_EXSTYLE, GetWindowLongPtr(hwnd, GWL_EXSTYLE) & ~WS_EX_LAYERED);
	RedrawWindow(hwnd, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN);
}
void WindowImplWin32::setAlpha(float alpha = 1.0f)
{
	HWND hwnd = getSystemHandle();
	if (alpha >= 1.0f)
	{
		makeOpaque(hwnd);
		return;
	...
	else
	{
		...
		SetLayeredWindowAttributes(getSystemHandle(), 0, BYTE(alpha * 255), LWA_ALPHA);
	}
}
