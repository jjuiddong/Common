...
bool ImGui::ButtonOption(const char* label, const ImVec2& size_arg, const bool enable)
{
    return ButtonEx(label, size_arg, enable? 0 : ImGuiButtonFlags_Disabled);
}
...
bool ImGui::BeginCombo2(const char* label, const char* preview_value, const float scale, const ImVec2 &offset, ImGuiComboFlags flags)
{
    // Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ImGuiContext& g = *GImGui;
    bool has_window_size_constraint = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) != 0;
...
bool ImGui::DragDouble(const char* label, double* v, double v_speed, double v_min, double v_max, const char* format, double power)
{
