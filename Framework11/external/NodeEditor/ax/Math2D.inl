...
inline typename ax::basic_rect<T>::point_t ax::basic_rect<T>::get_closest_point(const point_t& p, bool on_edge, float radius) const
{
    ...
    return point + static_cast<basic_point<T>>(min(distance, static_cast<float>(radius)) * static_cast<basic_point<float>>(offset) * (1.0f / distance));
}
...
inline typename ax::basic_rect<T>::point_t ax::basic_rect<T>::get_closest_point(const basic_rect& r) const
{
    ...
    if (r.left() >= right())
        ...
        result.x = (max(left(), r.left()) + min(right(), r.right())) / 2;
    ...
    if (r.top() >= bottom())
        ...
        result.y = (max(top(), r.top()) + min(bottom(), r.bottom())) / 2;
...
{
    rounding = min(rounding, min(w, h) / 2);
