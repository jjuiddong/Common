...
void ImGui::PlotEx2(ImGuiPlotType plot_type, const char* label, float(*values_getter)(void* data, int idx)
	, void* data, int values_count, int values_offset, int text_value_offset
	, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
	...
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(graph_size.x, graph_size.y));
	const ImRect inner_bb(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding - ImVec2(0,15)); // add x-axis text size
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, 0, &frame_bb))
		return;
	...
	if (values_count > 0)
	{
		...
		if (hovered)
		{
			...
			const float v1 = values_getter(data, (v_idx + 1 + values_offset) % values_count);
			if (plot_type == ImGuiPlotType_Lines)
				SetTooltip("%d: %8.4g\n%d: %8.4g", v_idx + text_value_offset, v0, v_idx + 1 + text_value_offset, v1);
			else if (plot_type == ImGuiPlotType_Histogram)
				SetTooltip("%d: %8.4g", v_idx, v0);
			v_hovered = v_idx;
		}
		...
		// render base line
		{
			int cnt = 0;
			float t0 = 0.f;
			const float l_step = 10.f / res_w;

			// Vertical Line
			while (t0 < 1)
			{
				const float t1 = t0 + l_step;
				const ImVec2 tp0 = ImVec2(t0, 1.0f);
				const ImVec2 tp1 = ImVec2(t0, 0.0f);
				ImVec2 pos0 = ImLerp(inner_bb.Min, inner_bb.Max, tp0);
				ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max, tp1);
				if (plot_type == ImGuiPlotType_Lines)
				{
					const ImVec4 color = ((cnt++ % 5) == 0)? ImVec4(0.3f, 0.3f, 0.3f, 0.3f) : ImVec4(0, 0, 0, 0.3f);
...

		{
			...
			int cnt = 0;
			float t0 = 0.f;
			const float h_step = 10.f / graph_size.y;

			while (t0 < 1)
			{
				const float t1 = t0 + h_step;
				const ImVec2 tp0 = ImVec2(0, t0);
				const ImVec2 tp1 = ImVec2(1, t0);
				ImVec2 pos0 = ImLerp(inner_bb.Min, inner_bb.Max, tp0);
				ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max, tp1);
				if (plot_type == ImGuiPlotType_Lines)
				{
...

		{
			...
			{
				const float t1 = t0 + l_step;
				const int v1_idx = (int)(t0 * item_count + 0.5f) + text_value_offset;
				const ImVec2 tp0 = ImVec2(t0, 1.0f);
				ImVec2 pos0 = ImLerp(inner_bb.Min, inner_bb.Max, tp0);
				if (plot_type == ImGuiPlotType_Lines)
				{
					...
					{
						char buff[32];
						sprintf(buff, "%d", v1_idx);
						RenderText(ImVec2(pos0.x, pos0.y), buff);
					}
				}
				t0 = t1;
			}
...

		{
			...
			{
				const float t1 = t0 + h_step;
				const ImVec2 tp0 = ImVec2(0, 1.f - t0);
				const float v1 = t0 * (scale_max - scale_min) + scale_min;
				ImVec2 pos0 = ImLerp(inner_bb.Min, inner_bb.Max, tp0);
				if (plot_type == ImGuiPlotType_Lines)
				{
					if (((cnt++ % 5) == 0) && (cnt != 1)) // ignore first y
					{
						char buff[32];
						sprintf(buff, "%.1f", v1);
						RenderText(ImVec2(pos0.x+5, pos0.y - 15), buff);
					}
				}
...
void ImGui::PlotLines2(const char* label, const float* values, int values_count, int values_offset, int text_value_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
	ImGuiPlotArrayGetterData data(values, stride);
	PlotEx2(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, text_value_offset, overlay_text, scale_min, scale_max, graph_size);
}
