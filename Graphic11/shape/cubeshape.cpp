...
// Create Cube
bool cCubeShape::Create1(cRenderer &renderer
...
{
	return CreateShape2(renderer, g_cubeVertices2, g_cubeIndices2, g_cubeUV2, colors, vtxType);
}
...
)
{
	...
	for (int i = 0; i < 12; ++i)
	{
		...
		Vector3 v2 = vertices[indices[i * 3 + 1]];
		Vector3 v3 = vertices[indices[i * 3 + 2]];
		cColor c1 = colors[indices[i * 3]];
		cColor c2 = colors[indices[i * 3 + 1]];
		cColor c3 = colors[indices[i * 3 + 2]];
		Vector2 uv1 = uvs[i * 3];
		Vector2 uv2 = uvs[i * 3 + 1];
		Vector2 uv3 = uvs[i * 3 + 2];
		Vector3 normal = (v2 - v1).Normal().CrossProduct((v3 - v1).Normal());
		...
		if (vtxType & eVertexType::NORMAL)
			*(Vector3*)(pvtx + normOffset) = normal;
		if (vtxType & eVertexType::COLOR)
			*(Vector4*)(pvtx + colorOffset) = c1.GetColor();
		if (vtxType & eVertexType::TEXTURE0)
			*(Vector2*)(pvtx + texOffset) = uv1;
		pvtx += vertexStride;
		...
		if (vtxType & eVertexType::NORMAL)
			*(Vector3*)(pvtx + normOffset) = normal;
		if (vtxType & eVertexType::COLOR)
			*(Vector4*)(pvtx + colorOffset) = c2.GetColor();
		if (vtxType & eVertexType::TEXTURE0)
			*(Vector2*)(pvtx + texOffset) = uv2;
		pvtx += vertexStride;
		...
		if (vtxType & eVertexType::NORMAL)
			*(Vector3*)(pvtx + normOffset) = normal;
		if (vtxType & eVertexType::COLOR)
			*(Vector4*)(pvtx + colorOffset) = c3.GetColor();
		if (vtxType & eVertexType::TEXTURE0)
			*(Vector2*)(pvtx + texOffset) = uv3;
		pvtx += vertexStride;
	}
...
void cCubeShape::SetCube(cRenderer &renderer, const Vector3 vertices[8]
	, const cColor colors[8])
{
	SetShape2(renderer, vertices, g_cubeIndices2, colors);
}
...
)
{
	...
	if (BYTE *initVertices = (BYTE*)m_vtxBuff.Lock(renderer))
	{
		...
		for (int i = 0; i < 12; ++i)
		{
			...
			Vector3 v3 = vertices[indices[i * 3 + 2]];
			Vector3 normal = (v2 - v1).Normal().CrossProduct((v3 - v1).Normal());
			const Vector4 c1 = colors[indices[i * 3]].GetColor();
			const Vector4 c2 = colors[indices[i * 3 + 1]].GetColor();
			const Vector4 c3 = colors[indices[i * 3 + 2]].GetColor();
