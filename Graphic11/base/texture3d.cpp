...
cTexture3d::cTexture3d(ID3D11ShaderResourceView *srv)
	: m_texSRV(srv)
...
bool cTexture3d::Create(cRenderer &renderer, const StrPath &fileName)
{
	Clear();
...
void cTexture3d::Bind(cRenderer &renderer
	, const int stage //= 0
)
{
...
void cTexture3d::Bind(cShader &shader, const Str32 &key)
{
	assert(0);
...
void cTexture3d::Unbind(cRenderer &renderer, const int stage)
{
...
void* cTexture3d::Lock(cRenderer &renderer, OUT D3D11_MAPPED_SUBRESOURCE &out)
{
...
void cTexture3d::Unlock(cRenderer &renderer)
{
	renderer.GetDevContext()->Unmap(m_texture, 0);
}


void cTexture3d::Clear()
{
	m_fileName.clear();
...
// 텍스쳐를 파일에 저장한다.
bool cTexture3d::WritePNGFile(cRenderer &renderer, const StrPath &fileName)
{
	return true;
	//const HRESULT result = DirectX::SaveWIcTexture3dToFile(renderer.GetDevContext()
...
bool cTexture3d::DrawText(cRenderer &renderer, const wchar_t *szText
...
int cTexture3d::Width()
{
	return m_imageInfo.Width;
}


int cTexture3d::Height()
{
	return m_imageInfo.Height;
}


bool cTexture3d::IsLoaded()
{
...
StrPath cTexture3d::ConvertTextureFileName(const char *fileName)
{
