...
{
	In.PosH.xy /= In.PosH.w;
	In.PosH.x = 0.5f * In.PosH.x + 0.5f;
	In.PosH.y = -0.5f * In.PosH.y + 0.5f;

	float4 color = GetLightingColor(In.Normal, In.toEye, 1.f);
	float4 texColor = txDiffuse.Sample(samLinear, In.Tex);
	float4 reflectColor = txReflect.Sample(samLinear, In.PosH.xy);
	float4 Out = color * lerp(texColor, reflectColor, gReflectionAlpha);
