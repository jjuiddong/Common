...
//------------------------------------------------------------------------
controlserver.s2c_Dispatcher = class {
	...
	dispatch(wss, ws, packet, handlers) {
		if (this.isRelay) {
			handlers.forEach(handler => {
				if (handler instanceof controlserver.s2c_ProtocolHandler)
...
//------------------------------------------------------------------------
controlserver.c2s_Dispatcher = class {
	constructor(isRelay = false) {
	...
	dispatch(wss, ws, packet, handlers) {
		if (this.isRelay) {
			handlers.forEach(handler => {
				if (handler instanceof controlserver.c2s_ProtocolHandler)
					handler.Relay(wss, ws, packet)
			})
			return
		}
...
//------------------------------------------------------------------------
controlserver.s2c_Protocol = class {
	constructor() { }
	...
	ReqStartCluster(ws, isBinary, name, url, projectName, mapFileName, pathFileName, taskName, ) {
		if (isBinary) { // binary send?
			let packet = new Packet(512)
			packet.pushStr(name)
			packet.pushStr(url)
...
//------------------------------------------------------------------------
controlserver.c2s_Protocol = class {
	constructor() { }
...
//------------------------------------------------------------------------
controlserver.s2c_ProtocolHandler = class {
	constructor() {} 
	Relay(wss, ws, packet) {} 
...
//------------------------------------------------------------------------
controlserver.c2s_ProtocolHandler = class {
	constructor() {} 
	Relay(wss, ws, packet) {} 
