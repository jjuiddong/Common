...
Vector2 CRSpline::Eq(float t, const Vector2& p1, const Vector2& p2, const Vector2& p3, const Vector2& p4)
{
	float t2 = t * t;
	float t3 = t2 * t;
...
void CRSpline::AddSplinePoint(const Vector2& v)
{
	vp.push_back(v);
	delta_t = (float)1 / (float)vp.size();
}

Vector2 CRSpline::GetInterpolatedSplinePoint(float t)
{
...
Vector2& CRSpline::GetNthPoint(int n)
{
	return vp[n];
}
