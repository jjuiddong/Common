...
void CCamWindow::OnDraw(CDC* pDC)
{
	...
	g.DrawImage(m_bitmap, Rect(0,0,cr.Width(), cr.Height()));
}
...
void CCamWindow::Render(cv::Mat &image)
{
	...
	//m_bound = Gdiplus::Rect(0, 0, m_bitmap->GetWidth(), m_bitmap->GetHeight());
	m_bound = Gdiplus::Rect(0, 0, cr.Width(), cr.Height());
