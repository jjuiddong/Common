...
cCube4::cCube4(cRenderer &renderer
	, const Vector3 &vMin
	, const Vector3 &vMax
...
void cCube4::InitCube(cRenderer &renderer
	, const float uvFactor //= 1.f
)
{
...
void cCube4::InitCube2(cRenderer &renderer, Vector3 vertices[8]
	, const float uvFactor //= 1.f
)
{
	//        4 --- 5
...
void cCube4::InitCube2(cRenderer &renderer
	, const float uvFactor //= 1.f
)
{
	if (m_vtxBuff.GetVertexCount() > 0)
...
void cCube4::InitCube3(cRenderer &renderer
	, Vector3 vertices[24]
	, const float uvFactor //= 1.f
)
{
...
void cCube4::SetCube(cRenderer &renderer, const Vector3 &vMin, const Vector3 &vMax
	, const float uvFactor //= 1.f
)
{
	if (m_vtxBuff.GetVertexCount() <= 0)
...
void cCube4::SetCube(const Vector3 &vMin, const Vector3 &vMax
	, const float uvFactor //= 1.f
)
{
...
// Not Use Matrix44
void cCube4::SetCube2(const Vector3 &vMin, const Vector3 &vMax
	, const float uvFactor //= 1.f
)
{
	m_min = vMin;
...
void cCube4::SetCube(const cBoundingBox &bbox
	, const float uvFactor //= 1.f
)
{
...
bool cCube4::Render(cRenderer &renderer
...
)
{
	...
	const Matrix44 transform = m_transform.GetMatrix()*parentTm;
	m_shader->SetMatrix("g_mWorld", transform);
	m_shader->SetFloat("g_uvFactor", m_uvFactor);

	{
		...
		mWIT.Inverse2();
		mWIT.Transpose();
		m_shader->SetMatrix("g_mWIT", mWIT);
	}
...
void cCube4::SetColor(const DWORD color)
{
